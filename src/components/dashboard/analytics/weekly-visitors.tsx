...
export interface WeeklyVisitorsProps {
  data?: { name: string; v1: number; v2: number; fullMark: number }[];
}

export function WeeklyVisitors({ data = [] }: WeeklyVisitorsProps): React.JSX.Element {
  const chartSize = 320;
...
function AngleTick({ x, y, payload }: AngleTickProps): React.JSX.Element {
  const { value } = payload;
...
interface RadiusTickProps {
  x: number;
  y: number;
  payload: { value: string };
}

function RadiusTick({ x, y, payload }: RadiusTickProps): React.JSX.Element {
  const { value } = payload;
