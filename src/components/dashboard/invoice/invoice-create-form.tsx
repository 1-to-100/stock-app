...
// You might have noticed that the submit button is not inside the form. To make it work and keep using the form hook,
// you might want to move the form hook to a parent component (such as a context), and pass the form methods
// and state to this component.
...
export function InvoiceCreateForm(): React.JSX.Element {
  ...
  const onSubmit = React.useCallback(
    async (_: Values): Promise<void> => {
      try {
        // Make API request
        router.push(paths.dashboard.invoices.details('1'));
      } catch (err) {
        logger.error(err);
      }
    },
