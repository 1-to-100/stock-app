...
export interface PowerUsageTodayProps {
  data: { name: string; v1: number; v2: number }[];
}

export function PowerUsageToday({ data = [] }: PowerUsageTodayProps): React.JSX.Element {
  const breakpoint = useCurrentBreakpoint();
  const chartHeight = 300;
  const xTicksInterval = ({ xs: 4, sm: 3, md: 2, lg: 1, xl: 1 } as Record<Breakpoint, number>)[breakpoint] ?? 1;
...
interface TooltipContentProps {
  active?: boolean;
  payload?: { fill: string; name: string; payload: { name: string }; value: number }[];
  label?: string;
}
